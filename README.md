...
Its implementation will comprise the development from the user perspective (frontend) to the algorithm that will
return the
...
and a column width. The function returns the string, but with line breaks inserted at just the right places to 
make sure that no line is longer than the column width. You try to break lines at word boundaries. 
